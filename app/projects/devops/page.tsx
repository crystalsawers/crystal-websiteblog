...
const DevOps = () => {
  const [data, setData] = useState<DevOpsDocument[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const items: DevOpsDocument[] = querySnapshot.docs.map((doc) => {
          const data = doc.data() as DevOpsDocument;
          return {
            id: doc.id,
            type: data.type,
            title: data.title,
        ...
        setError('Error fetching DevOps data');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="lg:mx-auto lg:max-w-screen-lg lg:p-8">
      <h1 className="page-title">Operations, DevOps, and Security</h1>
      ...
      {data.length === 0 ? (
        <p className="text-center text-custom-green">No Operations, DevOps, and Security posts yet.</p>
      ) : (
        data.map((item) => {
          ...
          return (
            <div key={item.id} className="card">
              ...
              <a href={`/projects/devops/${item.id}`} className="card-link">
                Read more
              </a>
              {isAuthenticated && (
