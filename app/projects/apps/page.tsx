...
const DevOps = () => {
  const [data, setData] = useState<AppsDocument[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const items: AppsDocument[] = querySnapshot.docs.map((doc) => {
          const data = doc.data() as AppsDocument;
          return {
            id: doc.id,
            type: data.type,
            title: data.title,
        ...
        setError('Error fetching Apps data');
      } finally {
        setLoading(false);
      }
    };
          ...
          {data.map((item) => {
            ...
            return (
              <div key={item.id} className="card">
                ...
                <a href={`/projects/apps/${item.id}`} className="card-link">
                  Read more
                </a>
                {isAuthenticated && (
