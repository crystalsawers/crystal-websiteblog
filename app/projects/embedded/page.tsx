...
const Embedded = () => {
  const [data, setData] = useState<EmbeddedDocument[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const items: EmbeddedDocument[] = querySnapshot.docs.map((doc) => {
          const data = doc.data() as EmbeddedDocument;
          return {
            id: doc.id,
            type: data.type,
            title: data.title,
        ...
        setError('Error fetching Embedded data');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="lg:mx-auto lg:max-w-screen-lg lg:p-8">
      <h1 className="page-title">Embedded Systems</h1>
